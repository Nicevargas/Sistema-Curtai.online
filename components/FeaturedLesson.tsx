...
export default function FeaturedLesson({ lesson, loading }: FeaturedLessonProps) {
  ...
  if (loading) {
    return (
      <div className="px-4 sm:px-0">
        <div className="w-full aspect-video bg-slate-50 rounded-[32px] animate-pulse border border-slate-200" />
      </div>
    );
  }
  ...
  if (!lesson) {
    return (
      <div className="px-4 sm:px-0">
        <div className="p-12 rounded-[32px] bg-slate-50 border border-dashed border-slate-200 flex flex-col items-center justify-center text-center">
          <Sparkles className="size-12 text-slate-300 mb-4" />
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
        ...
        {lesson.pdf_url && (
          <motion.a
            ...
            whileHover={{ y: -2 }}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 hover:text-slate-900 hover:bg-slate-100 transition-all"
          >
        ...
        >
          <AnimatePresence mode="wait">
            {!isPlaying ? (
              ...
              >
                <Image 
                  src={getDirectDriveLink(lesson.capa_url) || "https://picsum.photos/seed/course/1280/720"}
                  alt={lesson.titulo}
                  fill
